...
export default class infoController{
    ...
    static async store(req,res){
        ...
        try{
            ...
            const [enviar] = await connectionx.execute("INSERT INTO bob (title,description,img) VALUES(?,?,?)",[title,description,img])
            console.log(enviar)
        }
        catch(error){
    ...
    static async details(req,res){
        ...
        try{
            ...
            connectionx = await mysql.createConnection(dab)
            const [AID] = await connectionx.execute("SELECT * FROM bob WHERE id = ?",[id])
            console.log(AID)
